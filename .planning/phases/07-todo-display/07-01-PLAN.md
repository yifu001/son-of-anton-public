...
**State structure reference (from src/classes/claudeState.class.js):**
- `state.todos` is keyed by sessionId (extracted from filename: `{sessionId}-agent-{agentId}.json`)
- Each `state.todos[sessionId]` is an array of todo objects with `status` field
- Status values: "in_progress", "pending", "completed"
- Access pattern: `state.todos[sessionId]` where sessionId comes from `window.terminalSessions[window.currentTerm]`
</context>
...
2. State handling (_onStateChange):
   ...
   - **Data access:** Get todos from `event.detail.todos[sessionId]` (NOT `state.todos[sessionId]`)
     - The event detail IS the state object from ClaudeStateManager
     - state.todos is keyed by sessionId (verified from claudeState.class.js lines 193-206)
     - Each todos[sessionId] is an array with objects having `status` field
   - If no todos or empty array: render empty state ("No tasks")
...
10. Empty state:
  ...
  <verify>
1. File exists: `test -f src/assets/css/mod_todoWidget.css && echo "CSS file exists"`
2. CSS syntax check: `npx stylelint src/assets/css/mod_todoWidget.css --fix` (or if stylelint not available, open file in browser devtools and check for parse errors)
3. Required content present: `grep -q "@keyframes todoSpinner" src/assets/css/mod_todoWidget.css && echo "Spinner animation present"`
  </verify>
...
2. Update `src/_renderer.js`:
   - **Position context:** ContextWidget is commented out (line 540). The last active widget in right column is:
     - Line 537: `window.mods.netstat = new Netstat("mod_column_right");`
     - Line 538: `window.mods.globe = new LocationGlobe("mod_column_right");`
     - Line 539: `window.mods.conninfo = new Conninfo("mod_column_right");`
     - Line 540: `// window.mods.context = new ContextWidget(...)` (COMMENTED OUT)
     - Line 541: `window.mods.agentList = new AgentList("mod_column_left");`
   - Add TodoWidget instantiation AFTER conninfo (line 539), BEFORE the commented ContextWidget:
     `window.mods.todoWidget = new TodoWidget("mod_column_right");`
   - This places TodoWidget as the 4th widget in right column, after conninfo
  </action>
  <verify>
Run app with `npm start` from project root. Verify:
1. TodoWidget appears in right column after conninfo (4th position)
2. No console errors related to TodoWidget
3. Widget shows "NO SESSION" or "No tasks" state initially
  </verify>
  <done>TodoWidget integrated - CSS loaded, JS loaded, widget instantiated in right column after conninfo</done>
</task>
...
2. Right column shows TodoWidget after conninfo (4th widget position)
