...
must_haves:
  truths:
    - "Polling errors in ClaudeStateManager are logged when debug=true"
    - "Directory scan errors in ClaudeStateManager are logged when debug=true"
  ...
  artifacts:
    - path: "src/classes/claudeState.class.js"
      provides: "Debug-gated error logging in all catch blocks"
      contains: "window.settings && window.settings.debug"
  ...
  key_links:
    - from: "claudeState.class.js catch blocks"
      to: "console.warn"
      via: "debug flag check"
      pattern: "if.*window\\.settings.*debug.*console\\.warn"
...
<task type="auto">
  ...
  <action>
Add debug-gated logging to all 8 empty catch blocks in claudeState.class.js. Use `console.warn` for expected/recoverable errors, `[ClaudeState]` prefix.
